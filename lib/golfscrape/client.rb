module Golfscrape
  ...
  class Client                
    ...
    def leaderboard
      response = Leaderboard.new
      url = "http://sports.yahoo.com/golf/pga/leaderboard"
      doc = Nokogiri::HTML(open(url))

      response.parse_tournament(doc)
      doc.css('div#leaderboardtable table.sportsTable').first.css('tbody tr').each do |player|
        begin
          ...
          @name = player.css('td.player').css('a').first.children.last.text
          ...
          response.results << Hashie::Mash.new(:position => @position, 
            :name => @name, :first_round => @first, :second_round => @second, 
            :third_round => @third, :fourth_round => @fourth, :today => @today, 
            :thru => @thru, :total => @total, :strokes => @strokes)
        rescue 
        end  
      end
